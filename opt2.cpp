...
double People::GetSpendSum() {
    double ret = 0;
    for (auto task : tasks) {
        ret += matrix[pid][task->type];
    }
    return ret / (480 * 3);
}
...
struct ScoreManager {
    map <int, double> spendTimes;
    double finishSum, R, L, score;
    double spendSum = 0.0, spend2Sum = 0.0;
    ...
    void Reset() {
        ...
        for (auto &it : peoples) {
            finishSum += it.GetSum();
            spendTimes[it.pid] += it.GetSpendSum();
        }
        for (auto &it : spendTimes) {
            spendSum += it.second;
            spend2Sum += it.second * it.second;
        }
    ...
    void Update() {
        double cnt = pset.size();
        R = minSum / (finishSum - startSum);
        double ave = spendSum / spendTimes.size();
        double s = ave * ave * cnt + spend2Sum - 2 * (spendSum * ave);
        L = sqrt(s / cnt);
        score = R * 1000 - L * 99;
    }

    void RemovePeople(int id) {
        finishSum -= peoples[id].GetSum();
        int pid = peoples[id].pid;
        spendSum -= spendTimes[pid];
        spend2Sum -= spendTimes[pid] * spendTimes[pid];
        spendTimes[pid] -= peoples[id].GetSpendSum();
        spendSum += spendTimes[pid];
        spend2Sum += spendTimes[pid] * spendTimes[pid];
        Update();
    }
...
void InsertTask(int id, Task *task) {
    scoreManager.RemovePeople(id);
    peoples[id].tasks.insert(task);
    scoreManager.InsertPeople(id);
}

void RemoveTask(int id, Task *task) {
    scoreManager.RemovePeople(id);
    peoples[id].tasks.erase(task);
    scoreManager.InsertPeople(id);
}
...
clock_t startClock;
void CheckTime() {
    if ((double)(clock() - startClock) / CLOCKS_PER_SEC > 60 * 60 * 1 + 60 * 5) {
        exit(0);
    }
}
                ...
                for (int i = 0; i < peoples.size(); i++) {
                    ...
                    for (Task *task : cur) {
                        ...
                        for (int j = 0; j < peoples.size(); j++) {
                            ...
                            InsertTask(j, task);
                            double curScore = scoreManager.score;
            ...
            while (true) {
                ...
                for (int i = 0; i < peoples.size(); i++) {
                    ...
                    for (Task *task : cur) {
                        ...
                        for (int j = 0; j < peoples.size(); j++) {
                            ...
                            for (Task *tt : jtasks) {
                                ...
                                if (curScore > bestScore) {
                                    ...
                                    printf("opt 2: %f (%f, %f) \n", bestScore, scoreManager.R, scoreManager.L);
                                }
                        ...
                        if (sel != -1) {
                            InsertTask(sel, task);
                            RemoveTask(sel, another);
                            InsertTask(i, another);
                        } else {
                            InsertTask(i, task);
                        }
                    }
